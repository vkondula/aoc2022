...
if __name__ == '__main__':
    ...
    while stack:
        snapshot = stack.pop()
        end_states.add((tuple(sorted(snapshot.opened)), snapshot.flown))
        for location, flow in flow_mapping.items():
            ...
            if snapshot.time + travel_time > 26:
                continue
            stack.append(
                Snapshot(
                    ...
                    snapshot.flown + (26 - travel_time - 1 - snapshot.time) * flow_mapping[location]
                )
            )
    end_states = list((a, b) for a, b in end_states if len(a) > 5)
    for i, (a_path, a_flown) in enumerate(end_states):
        for j in range(i + 1, len(end_states)):
            b_path, b_flown = end_states[j]
            if set(a_path) & set(b_path):
                continue
            final_max = max(final_max, a_flown + b_flown)
    print(final_max)
